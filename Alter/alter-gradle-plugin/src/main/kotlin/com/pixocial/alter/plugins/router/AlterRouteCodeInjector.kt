...
class AlterRouteCodeInjector(managerJarFile: File?, routeRootList: List<String>) {

    private var mInitClassFile: File? = managerJarFile
    private val mRouteRootList = routeRootList

    fun execute() {
        if (mInitClassFile != null && mRouteRootList.isNotEmpty()) {
            ...
            while (enumeration.hasMoreElements()) {
                ...
                if (AlterRouteTransform.REGISTER_CLASS_FILE_NAME == entryName) {
                    val classReader = ClassReader(inputStream)
                    // 构建一个ClassWriter对象，并设置让系统自动计算栈和本地变量大小
                    val classWriter = ClassWriter(ClassWriter.COMPUTE_MAXS)
                    val classVisitor = AlterRouteClassVisitor(mRouteRootList, classWriter)
                    //开始扫描class文件
